    ...
    module Controller
      ...
      def fetch_client_and_user_from_cookie
        if (hash_data = fb_cookie_hash) and
          fb_cookie_signature_correct?(fb_cookie_hash,Facebooker2Multi.secret)
      ...
      def fb_cookie_name
        return "#{Facebooker2Multi.cookie_prefix + Facebooker2Multi.app_id.to_s}"
      end
      ...
      def fb_signed_request_sig_valid?(sig,encoded)
        base64 = Base64.encode64(HMAC::SHA256.digest(Facebooker2Multi.secret,encoded))
      ...
      def fetch_client_and_user_from_signed_request
        if facebook_params[:oauth_token]
          ...
          if @_current_facebook_client
            ...
            return generate_signature(sig_hash, Facebooker2Multi.secret)
          end
        end
      end
      ...
      def oauth2_fetch_client_and_user_from_cookie
        ...
        authenticator = Mogli::Authenticator.new(Facebooker2Multi.app_id, Facebooker2Multi.secret, nil)
